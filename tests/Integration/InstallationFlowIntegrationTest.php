...
PHP;
    ...
    public function testCompleteInstallationFlowWithMigrations(): void
    {
        ...
        // Verify plugin config was saved
        $config = $this->configWriter->getConfig();
        $this->assertArrayHasKey($packageName, $config['installed']);
        $this->assertEquals('1.0.0', $config['installed'][$packageName]['version']);
    }
...
PHP
    ...
    public function testInstallationStoresCorrectMetadata(): void
    {
        ...
        $config = $this->configWriter->getConfig();
        $installed = $config['installed'][$packageName];
        
        $this->assertEquals('2.5.0', $installed['version']);
        $this->assertArrayHasKey('installed_at', $installed);
        $this->assertArrayHasKey('path', $installed);
        $this->assertArrayHasKey('migrations_executed', $installed);
    }
}
